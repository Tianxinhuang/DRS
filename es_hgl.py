...
##            sm.train()
    ...
    def get_local(self,x,errs):
        modelnum=np.shape(errs)[0]
        ptnum=np.shape(errs)[1]
        wik=np.zeros_like(errs)
##        print(np.shape(errs),np.shape(np.expand_dims(np.arange(ptnum),axis=-1)))
    ...
    def get_rk(self,errs,wg,wl):
        wg=np.reshape(wg,[-1,1])
        #wg=np.expand_dims(wg,axis=-1)
        wcvg=np.sum(wg*errs,axis=0,keepdims=True)#1*n
        #print(np.shape(wcvg))
        wcvl=np.matmul(wl,errs)#m*n
        pk=wcvg/(1e-8+wcvl)

        x=self.xt#n*d
        dismat=np.sqrt(np.sum(np.square(np.expand_dims(x,axis=1)-np.expand_dims(x,axis=0)),axis=-1))#n*n
        dismat=dismat+100*np.eye(np.shape(x)[0])
        rmax=np.expand_dims(np.min(dismat,axis=0),axis=0)/2#1*n
        #print(pk)
        #assert False
        #rk=np.zeros_like(pk)
        rk=rmax*(1-1/pk)
        pkcut=(pk>1).astype(np.float)
        rk=rk*pkcut
        return rk
    ...
    def predict_values(self,x):
        ...
        rk=self.get_rk(self.errs,wg,wl)#m*n
        dismat=np.sqrt(np.sum(np.square(np.expand_dims(x,axis=1)-np.expand_dims(self.xt,axis=0)),axis=-1))#m*n
        idx=np.expand_dims(np.argmin(dismat,axis=-1),axis=-1)
        #idx=list(idx)
        #print(np.shape(idx))
        idx=np.concatenate([np.expand_dims(np.arange(np.shape(x)[0]),axis=-1),idx],axis=-1)
        idx=np.transpose(idx)
        idx=tuple(idx.tolist())
        
        cutl=(dismat[idx]<=rk[idx]).astype(np.float)
        cutg=(dismat[idx]>rk[idx]).astype(np.float)
        w=wl*np.expand_dims(cutl,axis=-1)+wg*np.expand_dims(cutg,axis=-1)#m*ns
